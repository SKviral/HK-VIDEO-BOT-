...
def handle_message(message):
        ...
        if len(parts) > 1:
            ...
            if files:
                ...
                for f in files:
                    ...
                    try:
                        if f['type'] == 'document': 
                            res = bot.send_document(chat_id, f['file_id'], caption=caption, reply_markup=markup)
                        elif f['type'] == 'video': 
                            res = bot.send_video(chat_id, f['file_id'], caption=caption, reply_markup=markup)
                        elif f['type'] == 'photo': 
                            res = bot.send_photo(chat_id, f['file_id'], caption=caption, reply_markup=markup)
                        elif f['type'] == 'audio': 
                            res = bot.send_audio(chat_id, f['file_id'], caption=caption, reply_markup=markup)
                        if res:
    ...
    if text.startswith("/reply "):
        ...
        if len(parts) == 3:
            try:
                ...
                bot.send_message(chat_id, "❌ মেসেজ পাঠানো যায়নি।")
        return
    ...
    if step == "wait_restore" and message.document:
        try:
            ...
            bot.send_message(chat_id, f"❌ রিস্টোর ব্যর্থ হয়েছে! Error: {e}")
        return
