...
def _post_to_category(cat_id, mtype, mid, user, d_link, s_link):
    ...
    for ch in cat.get("channels", []):
        ...
        if ch_type == "premium":
            link = d_link
            links_str = "\n".join([link]*rpt)
            caption = f"{ph_t}🔗 <b>Direct Download:</b>\n{links_str}\n\n<i>🕐 {now_str}</i>{pf_t}".strip() if user.get("btn_link_in_caption",1) else f"{ph_t}{pf_t}".strip()
            ...
            link = s_link
            links_str = "\n".join([link]*rpt)
            caption = f"{ph_t}🔗 <b>Download Link:</b>\n{links_str}\n\n<i>🕐 {now_str}</i>{pf_t}".strip() if user.get("btn_link_in_caption",1) else f"{ph_t}{pf_t}".strip()
            ...
            for item in due:
                try:
                    ...
                    if cat_id:
                        ...
                        ad_links = "\n".join([s_link]*rpt)
                        ad_cap = f"{ph_t}🔗 <b>Download Link:</b>\n{ad_links}\n\n<i>🕐 {now_str}</i>{pf_t}".strip() if user.get("btn_link_in_caption",1) else ""
                        ad_mk = _build_post_markup(user, s_link, s_link)
                        pr_links = "\n".join([d_link]*rpt)
                        pr_cap = f"{ph_t}🔗 <b>Direct Download:</b>\n{pr_links}\n\n<i>🕐 {now_str}</i>{pf_t}".strip() if user.get("btn_link_in_caption",1) else ""
...
def cb(call):
    ...
    # ════════════════════════════════════════
        ...
        update_user(cid, {"step": f"wait_catadd_{ch_type5}_{cat_id6}"})
