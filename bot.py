...
def get_user(chat_id):
    ...
    # পুরনো ডাটাবেস আপডেট (Backward Compatibility)
    if "custom_buttons" not in user:
        users_col.update_one({"chat_id": chat_id}, {"$set": {"custom_buttons": []}})
        user["custom_buttons"] =[]
    return user
...
def execute_channel_post(chat_id, user, media_type, media_id):
    ...
    bot.send_message(chat_id, "✅ <b>অসাধারণ!</b> মিডিয়া ও ফাইলগুলো চালু থাকা চ্যানেলগুলোতে পোস্ট করা হয়েছে।")
...
def callback_handler(call):
    ...
    # --- Auto Post Channels Management (New Version) ---
        ...
        if ch:
            ...
            call.data = f"list_ch_{ch['type']}" # Refresh list
    ...
    # --- Advanced Settings ---
        ...
        user[key] = new_val # Local update for refresh
...
def handle_message(message):
    ...
    # Add Auto Channels Parsing (Name | ID)
...
@app.route('/')
def home():
    return "Bot is running beautifully with advanced features!"
